...
namespace RiddlesOfTheDuergarKingdom.Game
{
	...
	public class Engine : EamonRT.Game.Engine, Framework.IEngine
	{
		...
		public override void AddUniqueCharsToArtifactAndMonsterNames()
		{
			...
			var blacksmithSmockArtifact = gADB[68];
		...
		public override void InitMonsters()
		{
			...
			MacroFuncs.Add(1, () =>
			{
				var gradStudentCompanionMonster = gGameState != null ? gMDB[gGameState.GradStudentCompanionUid] : null;
			...
			MacroFuncs.Add(6, () =>
			{
				return gGameState != null && gGameState.VolcanoErupting ? "nobody will answer you." : "someone will arrive shortly.";
			});
			...
			MacroFuncs.Add(10, () =>
			{
				return gGameState != null && gGameState.WaiverSigned ? "So, unfortunately, I can't help you as you've already used your allotment!" : "However, if you were an independent contractor, I could probably help you!";
			});
			...
			gGameState.GradStudentCompanionUid = gCharacter.Gender == Gender.Female ? 8 : 9;
		}
		...
		public override IList<IMonster> GetHostileMonsterList(IMonster monster)
		{
			...
			if (monster.Uid == 16)
			{
				monsterList = monsterList.Where(m => m.Uid == gGameState.Cm).ToList();
			}

			// Only player can attack black spider

			else if (monster.Uid != gGameState.Cm)
			{
		...
		public virtual void RockCrusherDestroysContents(IRoom room)
		{
			...
			var rockCrusherArtifact = gADB[48];
			...
			foreach (var artifact in artifactList)
			{
				if (room.IsLit())
				{
					gOut.Print("{0} mangles {1} and tears {2} apart!", rockCrusherArtifact.GetTheName(true), artifact.GetTheName(buf: Globals.Buf01), artifact.EvalPlural("it", "them"));

					gOut.Print("{0} {1} destroyed!", artifact.GetTheName(true), artifact.EvalPlural("is", "are"));
				}
		...
		public virtual void RockGrinderDestroysContents(IRoom room)
		{
			...
			var rockGrinderArtifact = gADB[49];
			...
			foreach (var artifact in artifactList)
			{
				if (room.IsLit())
				{
					gOut.Print("{0} mutilates {1} beyond recognition!", rockGrinderArtifact.GetTheName(true), artifact.GetTheName(buf: Globals.Buf01));
		...
		public virtual void DebrisSifterVibratesContents(IRoom room, IArtifact artifact = null)
		{
			...
			var debrisSifterArtifact = gADB[50];
			...
			foreach (var artifact02 in artifactList)
			{
				if (artifact == null || artifact.Uid == artifact02.Uid)
				{
					...
					{
						gOut.Print("{0} vibrates {1} violently but to little harm!", debrisSifterArtifact.GetTheName(true), artifact02.GetTheName(buf: Globals.Buf01));
					}
				}
			}
		}
